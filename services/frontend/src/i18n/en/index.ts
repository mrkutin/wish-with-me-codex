export default {
  home: {
    heroTitle: 'Hi! This is Wish With Me — a wishlist service',
    heroSubtitle: 'Share your wishes with friends and get exactly the gifts you want. It\'s free!',
    createWishlist: 'Create Wishlist',
    howItWorks: 'How does it work?',
    step1Title: 'Create your wishlist',
    step1Desc: 'Add the gifts you want with links to online stores',
    step2Title: 'Share with friends',
    step2Desc: 'Send them the link and they can reserve gifts for you',
    step3Title: 'Done! Enjoy!',
    step3Desc: 'You get awesome gifts, and your friends never wonder "What to give?"',
    benefit1Title: 'Get only the gifts you actually want',
    benefit1Desc: 'No more silly surprises! Add a link and get exactly what you wished for.',
    benefit1Cta: 'I want a wishlist',
    benefit2Title: 'Help your friends: the "What to give?" problem is solved',
    benefit2Desc: 'Send them your wishlist link and they can easily pick and reserve any gift!',
    benefit3Title: 'Keep all your wishes in one place',
    benefit3Desc: 'No more forgetting where you saved that thing. Create unlimited wishlists!',
    finalCta: 'Create your wishlist',
    // Phone mockup
    phoneBirthdayWishlist: 'Birthday Wishlist',
    phoneNewYearGifts: 'New Year Gifts',
    // Step badges
    step: 'STEP',
    // Chat bubbles
    chatQuestion: 'What to give?',
    chatAnswer: "Here's my wishlist!",
    chatHappy: 'Perfect!',
    // Showcase cards
    showcaseBirthday: 'Birthday',
    showcaseNewYear: 'New Year',
    showcaseHousewarming: 'Housewarming',
    showcaseWedding: 'Wedding',
  },
  common: {
    appName: 'Wish With Me',
    loading: 'Loading...',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    create: 'Create',
    back: 'Back',
    next: 'Next',
    done: 'Done',
    error: 'Error',
    success: 'Success',
    confirm: 'Confirm',
    close: 'Close',
    notNow: 'Not now',
  },
  auth: {
    login: 'Sign In',
    logout: 'Sign Out',
    register: 'Sign Up',
    email: 'Email',
    password: 'Password',
    confirmPassword: 'Confirm Password',
    name: 'Name',
    forgotPassword: 'Forgot password?',
    noAccount: "Don't have an account?",
    hasAccount: 'Already have an account?',
    loginWith: 'Sign in with {provider}',
    orContinueWith: 'or continue with',
    invalidCredentials: 'Invalid email or password',
    emailTaken: 'This email is already registered',
    passwordTooShort: 'Password must be at least 8 characters',
    welcomeBack: 'Welcome back',
    loginSubtitle: 'Sign in to continue to your wishlists',
    createAccount: 'Create account',
    registerSubtitle: 'Start creating and sharing your wishlists',
    brandingDescription: 'Create wishlists, share with friends, and make gift-giving magical.',
    featureOffline: 'Works offline',
    featureShare: 'Easy sharing',
    featureSync: 'Syncs everywhere',
    showPassword: 'Show password',
    hidePassword: 'Hide password',
  },
  wishlists: {
    title: 'My Wishlists',
    myWishlists: 'My Wishlists',
    sharedWithMe: 'Shared with me',
    create: 'Create Wishlist',
    edit: 'Edit Wishlist',
    empty: "You don't have any wishlists yet",
    emptyHint: 'Create your first wishlist to get started',
    noSharedWishlists: 'No shared wishlists yet',
    noSharedWishlistsHint: 'When someone shares a wishlist with you, it will appear here',
    name: 'Name',
    description: 'Description',
    isPublic: 'Public wishlist',
    items: 'items | {count} item | {count} items',
    created: 'Wishlist created',
    updated: 'Wishlist updated',
    deleted: 'Wishlist deleted',
    deleteConfirm: 'Delete wishlist?',
    deleteMessage: 'Are you sure you want to delete "{name}"? This action cannot be undone.',
    removeBookmark: 'Remove from list',
    removeBookmarkConfirm: 'Remove shared wishlist?',
    removeBookmarkMessage: 'Are you sure you want to remove "{name}" from your shared wishlists?',
    bookmarkRemoved: 'Wishlist removed from your list',
    chooseIcon: 'Choose an icon',
    chooseColor: 'Choose a color',
    untitled: 'Untitled Wishlist',
  },
  items: {
    title: 'Items',
    add: 'Add Item',
    edit: 'Edit Item',
    addManually: 'Add Manually',
    addFromUrl: 'Add from URL',
    itemTitle: 'Title',
    description: 'Description',
    price: 'Price',
    currency: 'Currency',
    quantity: 'Quantity',
    url: 'Product URL',
    urlHint: 'Paste a product link from any online store',
    urlDescription: 'Product information will be automatically extracted',
    sourceUrl: 'Link to buy (optional)',
    sourceUrlHint: 'URL where this item can be purchased',
    image: 'Photo (optional)',
    uploadImage: 'Upload photo',
    imageTooLarge: 'Image is too large (max 5MB)',
    pending: 'Pending',
    resolving: 'Resolving',
    resolved: 'Resolved',
    resolveFailed: 'Failed to resolve',
    retry: 'Retry',
    retryFailed: 'Retry failed',
    viewSource: 'View source',
    empty: 'No items yet',
    emptyHint: 'Add your first item to get started',
    created: 'Item added',
    updated: 'Item updated',
    deleted: 'Item deleted',
    deleteConfirm: 'Delete item?',
    deleteMessage: 'Are you sure you want to delete "{title}"?',
    duplicateUrl: 'This item is already in your wishlist',
  },
  profile: {
    title: 'Profile',
    settings: 'Settings',
    bio: 'Bio',
    publicUrl: 'Public URL',
    connectedAccounts: 'Connected Accounts',
    language: 'Language',
    birthday: 'Date of birth',
  },
  errors: {
    generic: 'Something went wrong',
    network: 'Check your internet connection',
    notFound: 'Page not found',
    unauthorized: 'Authentication required',
    createFailed: 'Failed to create',
    updateFailed: 'Failed to update',
    deleteFailed: 'Failed to delete',
  },
  validation: {
    required: 'This field is required',
    email: 'Please enter a valid email',
    minLength: 'Minimum {min} characters required',
    maxLength: 'Maximum {max} characters allowed',
    minValue: 'Minimum value: {min}',
    invalidUrl: 'Please enter a valid URL',
    invalidHandle: 'Only lowercase letters, numbers, and hyphens',
  },
  oauth: {
    connect: 'Connect',
    disconnect: 'Disconnect',
    connected: 'Connected',
    notConnected: 'Not connected',
    accountLinked: '{provider} account linked successfully',
    accountCreated: 'Account created successfully',
    loginSuccess: 'Signed in successfully',
    authFailed: 'Authentication failed. Please try again.',
    invalidCallback: 'Invalid authentication response',
    emailConflict: 'Email {email} is already registered. Please sign in with your password to link {provider}.',
    alreadyLinked: '{provider} is already linked to another account',
    cantUnlinkLast: 'You cannot disconnect your only sign-in method. Add a password or another account first.',
  },
  sharing: {
    share: 'Share',
    shareWishlist: 'Share Wishlist',
    createLink: 'Create Link',
    linkType: 'Link type',
    linkTypeMark: 'Can mark items',
    linkTypeView: 'View only',
    linkTypeMarkShort: 'Mark',
    linkTypeViewShort: 'View',
    linkTypeMarkHint: 'Recipients can mark items they plan to gift',
    linkTypeViewHint: 'Recipients can only view the wishlist',
    copyLink: 'Copy link',
    showQr: 'Show QR Code',
    revoke: 'Revoke',
    linkCopied: 'Link copied to clipboard',
    linkCreated: 'Share link created',
    linkRevoked: 'Share link revoked',
    linkNotFound: 'Share link not found or expired',
    revokeConfirm: 'Revoke share link?',
    revokeMessage: 'People with this link will no longer be able to access this wishlist.',
    noLinks: 'No share links yet',
    noLinksHint: 'Create a link to share this wishlist with friends and family',
    expires7days: 'Expires in 7 days',
    expires30days: 'Expires in 30 days',
    expires90days: 'Expires in 90 days',
    neverExpires: 'Never expires',
    createdAt: 'Created {date}',
    expiresAt: 'Expires {date}',
    accessCount: '{count} views',
    sharedBy: "From {name}'s wishlist",
    wantedQuantity: 'Wanted: {count}',
    alreadyMarked: '{count} marked',
    markAsPurchased: "I'll get this",
    markQuantity: "I'll get {count}",
    markMore: 'Mark more',
    ofAvailable: 'of {available} available',
    youMarked: "You're getting {count}",
    fullyMarked: 'All marked',
    unmark: 'Unmark',
    yourMark: 'You: {count}',
    youMarkedThis: "You'll get this",
    buyItem: 'Buy',
    markedSuccess: 'Item marked as purchased',
    unmarkedSuccess: 'Item unmarked',
    quantityExceeds: 'Not enough items available',
    cannotMarkOwn: 'You cannot mark items on your own wishlist',
    emptyWishlist: 'This wishlist is empty',
  },
  offline: {
    youAreOffline: "You're offline",
    backOnline: 'Back online!',
    syncing: 'Syncing...',
    syncError: 'Sync error',
    syncComplete: 'All changes saved',
    statusOffline: 'Offline - changes will sync when online',
    statusSyncing: 'Syncing changes...',
    statusError: 'Sync error - tap to retry',
    statusPending: 'Changes pending sync',
    statusSynced: 'All synced',
    savedOffline: 'Saved offline',
    willSyncWhenOnline: 'Will sync when online',
    conflictResolved: 'Some changes were updated',
    conflictCaption: 'A newer version was found on the server',
    createdOffline: 'Created offline',
    createdOfflineCaption: 'Will sync when back online',
    realtimeConnected: 'Real-time sync active',
    realtimeDisconnected: 'Real-time sync disconnected',
    realtimeReconnecting: 'Reconnecting...',
  },
  pwa: {
    installTitle: 'Install Wish With Me',
    installDescription: 'Add to your home screen for quick access',
    install: 'Install',
    updateAvailable: 'Update available',
    updateDescription: 'A new version is available',
    updateNow: 'Update now',
    updateLater: 'Later',
  },
};
