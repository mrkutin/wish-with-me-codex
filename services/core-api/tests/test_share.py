...
# =============================================================================
# Tests: List Share Links
# =============================================================================


@pytest.mark.asyncio
async def test_list_shares_owner(
    ...
    """Test listing share links as the wishlist owner returns active links."""
    ...
    mock_couchdb.find.return_value = [share_link_mark, share_link_view]
...
async def test_list_shares_not_owner(
    ...
    """Test listing share links as non-owner returns 403."""
    wishlist_uuid = wishlist["_id"].split(":")[1]
...
@pytest.mark.asyncio
async def test_list_shares_wishlist_not_found(
    mock_couchdb: MagicMock,
    owner_user: dict[str, Any],
) -> None:
    """Test listing share links for non-existent wishlist returns 404."""
    ...
    with patch("app.routers.share.get_couchdb", return_value=mock_couchdb), \
        ...
        async with AsyncClient(
            ...
            response = await client.get(
                f"/api/v1/wishlists/{nonexistent_uuid}/share",
                headers=create_auth_header(owner_user),
            )
...
@pytest.mark.asyncio
async def test_list_shares_filters_expired(
    mock_couchdb: MagicMock,
    owner_user: dict[str, Any],
    wishlist: dict[str, Any],
    share_link_mark: dict[str, Any],
    ...
    """Test that expired share links are filtered out from the list."""
    wishlist_uuid = wishlist["_id"].split(":")[1]
    ...
    mock_couchdb.get.side_effect = mock_get
    # Return both active and expired shares from DB
    mock_couchdb.find.return_value = [share_link_mark, expired_share_link]
    ...
    assert response.status_code == 200
    data = response.json()
    # Only the non-expired share should be returned
    assert len(data["items"]) == 1
    assert data["items"][0]["token"] == share_link_mark["token"]


@pytest.mark.asyncio
async def test_list_shares_filters_revoked(
    mock_couchdb: MagicMock,
    owner_user: dict[str, Any],
    wishlist: dict[str, Any],
    share_link_mark: dict[str, Any],
) -> None:
    """Test that revoked share links are filtered by the query (not returned)."""
    wishlist_uuid = wishlist["_id"].split(":")[1]
    ...
    mock_couchdb.get.side_effect = mock_get
    # The find query includes revoked: False, so only active shares are returned
    ...
    data = response.json()
    assert len(data["items"]) == 1
    # Verify the find was called with revoked: False
    mock_couchdb.find.assert_called_once()
    call_args = mock_couchdb.find.call_args[0][0]
    assert call_args.get("revoked") is False
